...
let count = 0;
  ...
  const { name, from, to, price, currcapacity, maxcapacity, departureDateTime, arrivalDateTime } = flight;
  ...
  return (
    <div className="w-full rounded overflow-hidden shadow-lg bg-white mb-10">
      <div className="p-4 space-y-4">
        ...
        <p className="text-gray-700 mb-2">{from} - {to}</p>
        ...
        {!isUser && <p className="text-gray-700 mb-4">
          Capacity: {currcapacity}/{maxcapacity}
        </p>}
        <p className="text-gray-700 mb-4">Price: ${price}</p>
        {isUser && <p className="text-gray-700 mb-4">Total Price: ${price*count}</p>}

        {!isUser ? <div className="flex items-center mb-4">
          ...
          <div className="flex items-center border border-gray-300 py-1 px-2 rounded leading-tight focus:outline-none focus:border-blue-500">
            <button onClick={handleDecrement} className="text-gray-500 focus:outline-none">
              -
            </button>
            <span className="px-2">{quantity}</span>
            <button onClick={handleIncrement} className="text-gray-500 focus:outline-none">
              +
            </button>
          </div>
        </div> :  <p className="text-gray-700 mb-4">Total tickets: {count}</p>}
        <div className="flex justify-between">
          <div></div> {/* Empty div to push the content to the left */}
          {isUser ? <button onClick={handleCancelBooking} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
            Cancel Booking
          </button> : <button onClick={handleBookFlight} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
